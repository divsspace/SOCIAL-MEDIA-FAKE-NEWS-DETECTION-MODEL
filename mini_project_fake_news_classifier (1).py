...
class TrainingHistory:
  def __init__(self):
    """Initialize with current timestamp: 2025-04-03 16:22:11"""
    self.timestamp = "2025-04-03 16:22:11"
    self.user_login = "divsspace"
    self.train_losses = []
    self.val_losses = []
    self.test_losses = []  # Added
    self.train_accs = []
    self.val_accs = []
    self.test_accs = []    # Added
    self.learning_rates = []
    self.epochs = []
    self.best_epoch = 0
    self.best_val_acc = 0

  def update(self, epoch, train_loss, val_loss, test_loss, train_acc, val_acc, test_acc, lr):
  ...
  def plot_metrics(self):
    ...
    ax1.plot(self.epochs, self.test_losses, label='Test Loss', color='green', alpha=0.7)
    ...
    ax2.plot(self.epochs, self.test_accs, label='Test Accuracy', color='green', alpha=0.7)
    ...
    val_acc_smoothed = np.convolve(self.val_accs,
                                 ...
                                 mode='valid')
    test_acc_smoothed = np.convolve(self.test_accs,  # Added
                                  np.ones(window)/window,
                                  mode='valid')
    ax4.plot(self.epochs[window-1:], val_acc_smoothed,
            label='Validation', color='red', alpha=0.7)
    ax4.plot(self.epochs[window-1:], test_acc_smoothed,  # Added
            label='Test', color='green', alpha=0.7)
    ax4.set_title(f'Smoothed Accuracies (window={window})')
    ...
    print(f"Final Test Accuracy: {self.test_accs[-1]:.4f}")  # Added
  ...
  def save_history(self, path=None):
    ...
    history_data = {
        ...
        'test_losses': self.test_losses,  # Added
        'train_accs': self.train_accs,
        'val_accs': self.val_accs,
        'test_accs': self.test_accs,      # Added
...
class SmallDatasetTrainer:
   ...
   def test(self, data):
    """Dedicated test method with proper test-time behavior"""
...
if __name__ == "__main__":
    try:
        ...
        for epoch in range(config.EPOCHS):
            ...
            # Test (add this right after validation)
            test_loss, test_acc, test_uncertainty = trainer.test({
                'x': test_features,
            ...
            # Update the print progress section with test metrics (replace the existing print statements)
            print(f"\nEpoch {epoch:03d}:")
            print("Metrics:")
            print(f"{'Type':<8} {'Loss':>10} {'Accuracy':>10} {'Uncertainty':>12}")
            print("-" * 42)
            print(f"Train:   {train_loss:10.4f} {train_acc:10.4f}")
            print(f"Val:     {val_loss:10.4f} {val_acc:10.4f} {val_uncertainty:12.4f}")
            print(f"Test:    {test_loss:10.4f} {test_acc:10.4f} {test_uncertainty:12.4f}")
...
def _generate_accuracy_plot(self, history):
    plt.figure(figsize=(12, 6))
    plt.plot(history.epochs, history.test_accs, 'g-', label='Test Accuracy', linewidth=2)  # Add test accuracy
    plt.plot(history.epochs, history.val_accs, 'r-', label='Validation Accuracy', linewidth=2)
    plt.plot(history.epochs, history.train_accs, 'b--', label='Training Accuracy', linewidth=2)

    plt.title('Model Accuracy Progression Over Epochs', pad=20)
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend(frameon=True, facecolor='white', edgecolor='gray')

    save_path = os.path.join(self.viz_dir, f'accuracy_progress_{self.save_prefix}.png')
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
    print(f"Accuracy plot saved: {save_path}")

def _generate_training_summary(self, history):
    summary_data = {
        'Metric': [
            'Best Validation Accuracy',
            'Final Training Accuracy',
            'Final Validation Accuracy',
            'Final Test Accuracy',     # Add test accuracy
            'Total Epochs',
            'Best Epoch',
            'Final Learning Rate',
            'Test Uncertainty'         # Add test uncertainty
        ],
        'Value': [
            f"{history.best_val_acc:.4f}",
            f"{history.train_accs[-1]:.4f}",
            f"{history.val_accs[-1]:.4f}",
            f"{history.test_accs[-1]:.4f}",  # Add test accuracy
            len(history.epochs),
            history.best_epoch,
            f"{history.learning_rates[-1]:.6f}",
            f"{history.test_uncertainty:.4f}" if hasattr(history, 'test_uncertainty') else 'N/A'  # Add test uncertainty
        ]
    }

    df = pd.DataFrame(summary_data)
    save_path = os.path.join(self.tab_dir, f'training_summary_{self.save_prefix}.csv')
    df.to_csv(save_path, index=False)
    print(f"Training summary saved: {save_path}")
    return df

def _generate_graph_structure_summary(self, graph_stats):
    """Generate summary of graph structure statistics"""
    summary_data = {
        'Split': ['Train', 'Validation', 'Test'],
        'Edges': [
            graph_stats['train_edges'],
            graph_stats['val_edges'],
            graph_stats['test_edges']
        ],
        'Edge Type Distribution': [
            str(graph_stats['edge_type_distribution']['train']),
            str(graph_stats['edge_type_distribution']['val']),
            str(graph_stats['edge_type_distribution']['test'])
        ]
    }

    df = pd.DataFrame(summary_data)
    save_path = os.path.join(self.tab_dir, f'graph_structure_{self.save_prefix}.csv')
    df.to_csv(save_path, index=False)
    print(f"Graph structure summary saved: {save_path}")
    return df

def generate_comprehensive_results(self, trainer, test_results):
    """Generate both visualizations and tabular results"""
    print("\nStarting comprehensive results generation...")

    # Generate visualizations
    self._generate_accuracy_plot(trainer.history)
    self._generate_roc_curves(test_results)
    self._generate_confusion_matrix(test_results)
    self._generate_metrics_summary(test_results)

    # Generate tabular results
    self._generate_classification_metrics(test_results)
    self._generate_confusion_matrix_table(test_results)
    self._generate_detailed_metrics(test_results)
    self._generate_training_summary(trainer.history)

    # Add graph structure summary if available
    if 'graph_stats' in test_results:
        self._generate_graph_structure_summary(test_results['graph_stats'])

    print("\nAll results generated successfully!")

generate_all_results(trainer, test_results)
...
class TabularResultsVisualizer:
    def __init__(self):
        self.timestamp = "2025-04-03 17:06:15"
        self.user_login = "divsspace"
        ...
        self.output_dir = '/content/drive/MyDrive/archive2/tabular_visualizations'
        os.makedirs(self.output_dir, exist_ok=True)

        self.save_prefix = f"{self.timestamp.replace(' ', '_').replace(':', '-')}_{self.user_login}"
        print(f"\nInitializing tabular visualization at {self.timestamp}")
        print(f"User: {self.user_login}")

    def visualize_all_tables(self, test_results, trainer_history):
        """Generate visual representations of all tables"""
        print("\nGenerating tabular visualizations...")

        self._visualize_classification_metrics(test_results)
        self._visualize_confusion_matrix(test_results)
        self._visualize_detailed_metrics(test_results)
        self._visualize_training_summary(trainer_history)
        self._visualize_graph_structure(test_results)

        print("\nAll tabular visualizations completed!")

    def _create_table_image(self, data, title, fontsize=10):
        """Helper function to create table visualization"""
        # Calculate figure size based on data
        figsize = (12, max(len(data)*0.5 + 2, 6))
        ...
        table = ax.table(
            ...
            cellLoc='center',
            loc='center',
            cellColours=[['#f2f2f2']*len(data.columns)]*len(data),
        ...
        plt.title(title, pad=20, fontsize=fontsize+2)
        plt.figtext(0.02, 0.02, f'Generated: {self.timestamp}\nUser: {self.user_login}',
                   fontsize=8, alpha=0.7)

        return fig

    def _visualize_classification_metrics(self, test_results):
        """Create classification metrics table visualization"""
        classes = ['True', 'False', 'Unverified', 'Non-rumor']
        metrics_data = []

        for i, metrics in enumerate(test_results['per_class_metrics']):
            metrics_data.append({
                'Class': classes[i],
        ...
        fig = self._create_table_image(df, 'Classification Metrics by Class')

        save_path = os.path.join(self.output_dir, f'classification_metrics_table_{self.save_prefix}.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.show()
        plt.close()
        print(f"Classification metrics table visualization saved: {save_path}")

    def _visualize_confusion_matrix(self, test_results):
        """Create confusion matrix table visualization"""
        classes = ['True', 'False', 'Unverified', 'Non-rumor']
        df = pd.DataFrame(
        ...
        fig = self._create_table_image(df, 'Confusion Matrix')

        save_path = os.path.join(self.output_dir, f'confusion_matrix_table_{self.save_prefix}.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.show()
        plt.close()
        print(f"Confusion matrix table visualization saved: {save_path}")

    def _visualize_detailed_metrics(self, test_results):
        ...
        for i, metrics in enumerate(test_results['per_class_metrics']):
            detailed_metrics.append({
                ...
                'Avg Uncertainty': f"{metrics['avg_uncertainty']:.4f}",
                'Support': metrics['support'],
                'F1 Score': f"{metrics['f1']:.4f}"
            })

        df = pd.DataFrame(detailed_metrics)
        fig = self._create_table_image(df, 'Detailed Performance Metrics by Class')

        save_path = os.path.join(self.output_dir, f'detailed_metrics_table_{self.save_prefix}.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.show()
        plt.close()
        print(f"Detailed metrics table visualization saved: {save_path}")

    def _visualize_training_summary(self, trainer_history):
        """Create training summary table visualization"""
        summary_data = {
            'Metric': [
                'Best Validation Accuracy',
                'Final Training Accuracy',
            ...
            'Value': [
                f"{trainer_history.best_val_acc:.4f}",
                f"{trainer_history.train_accs[-1]:.4f}",
                f"{trainer_history.val_accs[-1]:.4f}",
                f"{trainer_history.test_accs[-1]:.4f}",
                len(trainer_history.epochs),
                trainer_history.best_epoch,
                f"{trainer_history.learning_rates[-1]:.6f}",
                f"{trainer_history.test_uncertainty:.4f}" if hasattr(trainer_history, 'test_uncertainty') else 'N/A'
            ]
        }

        df = pd.DataFrame(summary_data)
        fig = self._create_table_image(df, 'Training Summary')

        save_path = os.path.join(self.output_dir, f'training_summary_table_{self.save_prefix}.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.show()
        plt.close()
        print(f"Training summary table visualization saved: {save_path}")

    def _visualize_graph_structure(self, test_results):
        """Create graph structure table visualization"""
        if 'graph_stats' not in test_results:
            return

        graph_stats = test_results['graph_stats']
        summary_data = {
            'Split': ['Train', 'Validation', 'Test'],
            'Total Edges': [
                graph_stats['train_edges'],
        ...
        df = pd.DataFrame(summary_data)
        fig = self._create_table_image(df, 'Graph Structure Summary')

        save_path = os.path.join(self.output_dir, f'graph_structure_table_{self.save_prefix}.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.show()
        plt.close()
        print(f"Graph structure table visualization saved: {save_path}")

def generate_tabular_visualizations(trainer, test_results):
    """Generate visual representations of all tabular results"""
    try:
        visualizer = TabularResultsVisualizer()
        visualizer.visualize_all_tables(test_results, trainer.history)

        print("\nAll tabular visualizations generated successfully!")
        print(f"Results saved to: {visualizer.output_dir}")

    except Exception as e:
        print(f"\nError generating tabular visualizations: {str(e)}")
        import traceback
        traceback.print_exc()

# Example usage at the end of the training script
generate_tabular_visualizations(trainer, test_results)